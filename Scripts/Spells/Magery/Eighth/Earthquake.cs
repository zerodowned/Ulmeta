...
namespace Server.Spells.Eighth
{
	public class EarthquakeSpell : MagerySpell
	{
		...
		public override bool DelayedDamage { get { return !Core.SE; } }
			...
			if( SpellHelper.CheckTown( Caster, Caster ) && CheckSequence() )
			{
				...
				if( map != null )
					foreach( Mobile m in Caster.GetMobilesInRange( 1 + (int)(Caster.Skills[SkillName.Magery].Value / 15.0) ) )
						if( Caster != m && SpellHelper.ValidIndirectTarget( Caster, m ) && Caster.CanBeHarmful( m, false ) && (!Core.SE || Caster.InLOS( m )) )
							targets.Add( m );
