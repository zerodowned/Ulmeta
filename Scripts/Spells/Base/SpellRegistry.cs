            ...
            for( int i = 0; i < m_CircleNames.Length; ++i )
            {
                ...
                if( t != null && !t.IsSubclassOf(typeof(SpecialMove)) )
                {
                    ...
                    try
                    {
                        ...
                        if( caster != null )
                        {
                            if( caster.AccessLevel < AccessLevel.Counselor && caster.Backpack != null )
                            {
                                List<Spellbook> books = caster.Backpack.FindItemsByType<Spellbook>();

                                for( int j = 0; spell == null && j < books.Count; j++ )
                                {
                                    if( books[i].HasSpell(GetRegistryNumber(t)) )
