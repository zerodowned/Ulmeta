...
namespace Server.Gumps
{
		...
		public override void OnResponse( NetState sender, RelayInfo info )
		{
			if( info.ButtonID == 1 )
			{
				try
				{
					if( m_From.AccessLevel >= AccessLevel.GameMaster )
					{
						...
						if( text != null )
						{
							if( m_Caps )
							{
								...
								CommandLogging.LogChangeProperty( m_From, m_Target, String.Format( "{0}.Cap", m_Skill ), m_Skill.Cap.ToString() );
							}
							else
							{
					...
					m_From.SendGump( new SkillsGump( m_From, m_Target, m_Caps, m_Selected ) );
				}
				catch
				{
					m_From.SendMessage( "Bad format. ###.# expected." );
					m_From.SendGump( new EditSkillGump( m_From, m_Target, m_Skill, m_Caps, m_Selected ) );
				}
			}
			else
			{
	...
	public class SkillsGump : Gump
	{
				...
				case 0:
					{
						if( index >= 0 && index < m_Groups.Length )
						{
							...
							if( m_Selected != newSelection )
								m_From.SendGump( new SkillsGump( m_From, m_Target, m_Caps, newSelection ) );
							else
								m_From.SendGump( new SkillsGump( m_From, m_Target, m_Caps, null ) );
						}
				...
				case 1:
					{
						if( m_Selected != null && index >= 0 && index < m_Selected.Skills.Length )
						{
							...
							if( sk != null )
							{
								if( m_From.AccessLevel >= AccessLevel.GameMaster )
								{
									m_From.SendGump( new EditSkillGump( m_From, m_Target, sk, m_Caps, m_Selected ) );
								}
								else
								{
									m_From.SendMessage( "You may not change that." );
		...
		public SkillsGump( Mobile from, Mobile target )
			: this( from, target, false, null )
		{
		}

		public SkillsGump( Mobile from, Mobile target, bool caps, SkillsGumpGroup selected )
			: base( GumpOffsetX, GumpOffsetY )
		{
			m_From = from;
			m_Target = target;
