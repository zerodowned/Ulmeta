...
namespace Server.Misc
{
    public class Titles
    {
        ...
        public static void AwardFame(Mobile m, int offset, bool message)
        {
            if (offset > 0)
            {
                if (m is Player && ((Player)m).FameLocked)
                    return;
        ...
        public static string ComputeTitle(Mobile beholder, Mobile beheld)
        {
            ...
            title.Append(GetNameTitle(beholder, beheld));
        ...
        /// <summary>
        /// Calculates the reputation title of the specified mobile
        /// </summary>
        /// <returns>the <code>Mobile</code>'s reputation title</returns>
        public static string GetNameTitle(Mobile beholder, Mobile beheld)
        {
            ...
            if (beheld.ShowFameTitle || (beholder == beheld))
            {
                for (int i = 0; i < m_FameEntries.Length; i++)
                {
                    ...
                    if (beheld.Fame <= fe.m_Fame || i == (m_FameEntries.Length - 1))
                    {
                        ...
                        for (int j = 0; j < karmaEntries.Length; j++)
                        {
                            ...
                            if (beheld.Karma <= ke.m_Karma || (j == karmaEntries.Length - 1))
                            {
                                builder.AppendFormat(ke.m_Title, beheld.Name, beheld.Female ? "Lady" : "Lord");
                                break;
                            }
                        }
        ...
        public static string GetSkillTitle(Mobile mob)
        {
            ...
            if (highest != null && highest.BaseFixedPoint >= 300)
            {
                ...
                if (mob.Female)
                {
                    if (skillTitle == "Swordsman")
                        skillTitle = "Swordmaiden";
                    else if (skillTitle.EndsWith("man"))
