...
namespace Server.Engines.PartySystem
{
	...
	public sealed class PartyTextMessage : Packet
	{
			...
			: base( 0xBF )
		{
			...
			Server.Utilities.Logging.SpeechLogging.WriteLine( from, String.Format( "[PARTY] {0}: {1}", String.Format( "{0} ({1})", from.RawName, from.Account == null ? "null" : ((Account)from.Account).Username ), text ) );
		}
	}
