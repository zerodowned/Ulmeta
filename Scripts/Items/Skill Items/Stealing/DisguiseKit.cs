...
namespace Server.Items
{
		...
		public override void OnResponse( NetState sender, RelayInfo info )
		{
			if( info.ButtonID == 0 )
			{
				if( m_Used )
                    ...
                    m_From.SendMessage("Your disguise will wear off in {0} hours", (int)(m_From.Skills.Forensics.Value / 20)); // Disguises wear off after 2 hours.
				else
			...
			if( index >= 0 && index < entries.Length )
			{
                ...
                m_From.SendMessage("Your disguise will wear off in {0} hours", (int)(m_From.Skills.Forensics.Value / 20));
                DisguiseTimers.CreateTimer(m_From, TimeSpan.FromHours((int)(m_From.Skills.Forensics.Value / 20)));
				DisguiseTimers.StartTimer( m_From );
                m_From.SendGump(new DisguiseNameGump(m_From));
    ...
    public class DisguiseNameGump : Gump
    {
        private Mobile m_from;

        public DisguiseNameGump(Mobile from)
            : base(50, 50)
        {
            m_from = from;
            ...
            AddPage(0);
            AddBackground(69, 98, 336, 148, 9200);
            AddTextEntry(170, 170, 200, 20, 0, 0, @"", 50);
            AddLabel(135, 130, 0, @"Please Select A New Name.");
            AddButton(90, 165, 247, 248, (int)Buttons.OkayButton, GumpButtonType.Reply, 0);
            AddItem(335, 125, 3589);
            AddImage(365, 200, 59);
            AddImage(90, 200, 57);
                ...
                case (int)Buttons.OkayButton:
                    {
                        ...
                        string m_nameMod = (entry0 == null ? "" : entry0.Text.Trim());
                        if (!String.IsNullOrEmpty(m_nameMod))
                        {
                            from.NameMod = m_nameMod;
                        }
