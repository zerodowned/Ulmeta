...
namespace Server.Items
{
	public abstract class BasePoleArm : BaseMeleeWeapon, IUsesRemaining
	{
		...
		public override void OnHit( Mobile attacker, Mobile defender, double damageBonus )
		{
			...
			if ( Core.AOS && (attacker.Player || attacker.Body.IsHuman) && Layer == Layer.TwoHanded && (attacker.Skills[SkillName.Anatomy].Value / 1000.0) >= Utility.RandomDouble() )
			{
				...
				if ( mod == null )
				{
					...
					defender.AddStatMod( new StatMod( StatType.Int, "Concussion", -(defender.RawInt / 4), TimeSpan.FromSeconds( 3.0 ) ) );
