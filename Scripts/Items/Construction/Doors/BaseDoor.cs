...
namespace Server.Items
{
	...
	public abstract class BaseDoor : Item, ILockable, ITelekinesisable, ILockpickable, ITrappable
	{
		...
		private static void EventSink_OpenDoorMacroUsed( OpenDoorMacroEventArgs args ) {
			...
			if( m.Map != null ) {
				...
				foreach( Item item in sector.Items ) {
					if( item.Location.X == x && item.Location.Y == y && (item.Z + item.ItemData.Height) > m.Z && (m.Z + 16) > item.Z && (item is BaseDoor || item is TentFlap) && m.CanSee( item ) && m.InLOS( item ) ) {
						if( m.CheckAlive() ) {
							if( item is BaseDoor ) {
		...
		public BaseDoor( int closedID, int openedID, int openedSound, int closedSound, Point3D offset )
			...
			_timeToClose = TimeSpan.FromSeconds( 10.0 );
		...
		public override void GetContextMenuEntries( Mobile from, System.Collections.Generic.List<Server.ContextMenus.ContextMenuEntry> list ) {
			if( from.Alive )
				list.Add( new Server.ContextMenus.DoorKnockEntry( this ) );
