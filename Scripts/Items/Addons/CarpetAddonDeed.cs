...
namespace Server.Items
{
	public class CarpetAddonDeed : BaseAddonDeed
	{
		...
		{
			Name = "a carpet addon deed";
		}

		public CarpetAddonDeed( Serial serial )
			: base( serial )
		{
		}
		...
		private void BoundingBox_Callback( Mobile from, Map map, Point3D start, Point3D end, object state )
		{
			...
			if( from.AccessLevel <= AccessLevel.Counselor && (width < 2 || height < 2) )
...
namespace Server.Gumps
{
	public class CarpetGump : Gump
	{
		...
		public CarpetGump( BaseAddonDeed deed, IPoint3D p, Map map, int width, int height )
			: base( 30, 30 )
		{
			m_Deed = deed;
			m_P3D = p;
