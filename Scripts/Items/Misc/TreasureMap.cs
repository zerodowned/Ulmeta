...
namespace Server.Items
{
	public class TreasureMap : MapItem
	{
		...
		private static Type[][] m_SpawnTypes = new Type[][]
		{
			...
			new Type[]{ typeof( Server.Mobiles.BlackDragon ), typeof( Balron ), typeof( BloodElemental ), typeof( PoisonElemental ), typeof( Titan ) }
		};
		#endregion

		public const double LootChance = 0.01; // 1% chance to appear as loot

		#region Point Initialization
		...
		public TreasureMap( int level, Map map )
		{
			...
			//ToDo determine if this is the best way... maybe a static bounds surrounding the pin would be better
		...
		public static bool HasDiggingTool( Mobile m )
		{
			...
			List<BaseAxe> axes = m.Backpack.FindItemsByType<BaseAxe>();

			foreach( BaseAxe axe in axes )
			{
				if( (axe is Pickaxe) || (axe is SturdyPickaxe) )
					return true;
			}
