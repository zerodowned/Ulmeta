...
namespace Server.SkillHandlers
{
    public class Stealing
    {
        ...
        private class StealingTarget : Target
        {
                ...
                else
                {
                    ...
                    else
                    {
                        ...
                        if( stolen != null )
                        {
                            ...
                            if (m_Thief is Player)
                            {
                                Rogue rge = Perk.GetByType<Rogue>((Player)m_Thief);
                                bool reveal = true;

                                if (rge != null && rge.Unseen())
                                    reveal = false;
                            ...
                            #region caught chance
                            int noticeChance = Utility.Random(110);
                            List<Mobile> witnesses = new List<Mobile>();
                            int mobileCount = 0, guardCount = 0;

                            foreach( Mobile m in m_Thief.GetMobilesInRange(12) )
                            {
                                if( !(m == m_Thief) && (m.CanSee(m_Thief) || m.InLOS(m_Thief)) )
                                {
                                    if( m is PlayerMobile || m is BaseVendor )
                                    {
                                        mobileCount++;
                                        witnesses.Add(m);
                                    }
                            ...
                            //+1% for every player or vendor, +10% for every guard
                            noticeChance += (mobileCount + (guardCount * 10));

                            caught = ((m_Thief.Skills[SkillName.Stealing].Fixed / 10) < noticeChance);
                            #endregion

                            if( m_Thief.Player )
                            {
                                EventDispatcher.InvokePlayerSteal(new PlayerStealEventArgs((Player)m_Thief, stolen, (root as Mobile), caught, witnesses));
                            }
                        }
                        else
                        {
            ...
            protected override void OnTarget( Mobile from, object target )
            {
                ...
                EventSink.InvokeSkillUsed(new SkillUsedEventArgs(from, from.Skills[SkillName.Stealing]));
    ...
    {
        public static readonly TimeSpan StealTime = TimeSpan.FromMinutes(30.0);
