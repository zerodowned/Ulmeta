        ...
        private class InternalTarget : Target
        {
            ...
            protected override void OnTarget( Mobile src, object targ )
            {
                ...
                if( range > 0 )
                {
                    ...
                    {
                        if( trg.Hidden && src != trg && src.InLOS(trg) )
                        {
                            ...
                            if( src.AccessLevel >= trg.AccessLevel && (ss >= ts || (inHouse && house.IsInside(trg))) )
                            {
                                ...
                                Scout sct = Perk.GetByType<Scout>((Player)trg);

                                if (sct == null || !sct.Undetectable())
                                {
                                    trg.RevealingAction();
                ...
                EventSink.InvokeSkillUsed(new SkillUsedEventArgs(src, src.Skills[SkillName.DetectHidden]));
            }
        }
    }
}
