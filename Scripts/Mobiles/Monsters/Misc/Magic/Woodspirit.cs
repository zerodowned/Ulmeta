...
namespace Server.Mobiles {
	[CorpseName( "a spirit's corpse" )]
	public class Woodspirit : BaseCreature {
		[Constructable]
		public Woodspirit()
			: base( AIType.AI_Mage, FightMode.Aggressor, 10, 1, 0.2, 0.4 ) {
			Name = "a woodland spirit";
			Body = Utility.RandomList( 98, 205, 5, 63, 238, 52 );
			Hue = 1155;
		...
		private void DoAreaLeech_Finish() {
			...
			foreach( Mobile m in this.GetMobilesInRange( 6 ) ) {
				if( this.CanBeHarmful( m ) && this.IsEnemy( m ) && !(m == this.SummonMaster || m == this.ControlMaster) )
					list.Add( m );
			}
		...
		public Woodspirit( Serial serial )
			: base( serial ) {
		}
