...
namespace Server.Mobiles
{
	public class BaseEscortable : BaseCreature
	{
		...
		private static string[] m_TownNames = new string[]
			{
				"Caer Picaren", "Noripsni", "Resara",
				"Gi'yrn Lake City", "Nayt'silm City", "Serool Fort",
				"Altru", "Kyrline",
				"Rifleagh", "Doac'lyn"
			};
		...
		{
			if( Map.Backtrol.Regions.Count == 0 || Map == null || Map == Map.Internal || Location == Point3D.Zero )
		...
		public EDI GetDestination()
		{
			...
			if( Map.Backtrol.Regions.Count > 0 )
	...
	public class EscortDestinationInfo
	{
		...
		{
			ICollection list = Map.Backtrol.Regions.Values;
