...
namespace Server.Mobiles {
	public class AnimalSkittishAI : BaseAI {
		public AnimalSkittishAI( BaseCreature m )
			: base( m ) {
		}
		...
		public override bool DoActionWander() {
			...
			distance < 8 && Utility.RandomDouble() * Math.Sqrt( (8 - distance) / 6 ) >= incoming.Skills[SkillName.AnimalTaming].Value )
			...
			*/
			...
			} else if( AcquireFocusMob( m_Mobile.RangePerception, m_Mobile.FightMode, false, false, true ) ) {  // if the mobile initiates combat, flee...
				if( m_Mobile.Debug )
