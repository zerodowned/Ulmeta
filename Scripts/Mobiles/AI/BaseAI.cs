    ...
    public abstract class BaseAI
    {
        ...
        public virtual void EndPickTarget( Mobile from, Mobile target, OrderType order )
        {
            ...
            if( order == OrderType.Attack )
            {
                ...
                if (target is Player)
                {
                    Beastmaster bmr = Perk.GetByType<Beastmaster>((Player)target);
                    if (bmr != null && bmr.IsItimidating())
                    {
                        m_Mobile.SayTo(from, "Your pet refuses to attack this creature!");
                        return;
                    }
        ...
        public virtual void OnCurrentOrderChanged()
        {
            ...
            switch( m_Mobile.ControlOrder )
            {
                ...
                case OrderType.Release:
                    ...
                    EventDispatcher.InvokeCreatureTameStateChange(new CreatureTameStateChangeArgs(m_Mobile.ControlMaster as Player, m_Mobile, false));
                    break;
        ...
        public virtual bool DoOrderCome()
        {
            if( m_Mobile.ControlMaster != null && !m_Mobile.ControlMaster.Deleted )
            {
                ...
                else
                {
                    ...
                    Direction dir = m_Mobile.ControlMaster.Direction;
                    bool ownerMounted = (m_Mobile.ControlMaster.Mount != null);
                    bool running = ((dir & Direction.Running) != 0);
                    bool walkRun;

                    if( ownerMounted )
                    {
                        m_Mobile.CurrentSpeed = running ? 0.1 : 0.15;
                        walkRun = WalkMobileRange(m_Mobile.ControlMaster, 2, true, 0, 1);
                    }
                    else
                    {
                        m_Mobile.CurrentSpeed = running ? 0.18 : 0.22;
                        walkRun = WalkMobileRange(m_Mobile.ControlMaster, 2, running, 0, 1);
                    }
        ...
        public virtual bool DoOrderFollow()
        {
            ...
            else if( m_Mobile.ControlTarget != null && !m_Mobile.ControlTarget.Deleted && m_Mobile.ControlTarget != m_Mobile )
            {
                ...
                else
                {
                    ...
                    Direction dir = m_Mobile.ControlTarget.Direction;
                    bool ownerMounted = (m_Mobile.ControlTarget.Mount != null);
                    bool running = ((dir & Direction.Running) != 0);
                    bool walkRun;
                    ...
                    {
                        m_Mobile.CurrentSpeed = running ? 0.1 : 0.15;
                        walkRun = WalkMobileRange(m_Mobile.ControlTarget, 2, true, 0, 1);
                    }
                    else
                    {
                        m_Mobile.CurrentSpeed = running ? 0.18 : 0.22;
                        walkRun = WalkMobileRange(m_Mobile.ControlTarget, 2, running, 0, 1);
                    }
        ...
        public virtual bool DoOrderGuard()
        {
            ...
            else
            {
                ...
                Direction dir = controlMaster.Direction;
                bool ownerMounted = (controlMaster.Mount != null);
                bool running = ((dir & Direction.Running) != 0);

                if( ownerMounted )
                {
                    m_Mobile.CurrentSpeed = running ? 0.1 : 0.15;
                    WalkMobileRange(controlMaster, 2, true, 0, 1);
                }
                else
                {
                    m_Mobile.CurrentSpeed = running ? 0.18 : 0.22;
                    WalkMobileRange(controlMaster, 2, running, 0, 1);
                }
            }
        ...
        public virtual bool AcquireFocusMob( int iRange, FightMode acqType, bool bPlayerOnly, bool bFacFriend, bool bFacFoe )
        {
            ...
            if( map != null )
            {
                ...
                foreach( Mobile m in eable )
                {
                    ...
                    // Staff members cannot be targeted **unless they have targeted us!!**
                    if( m.AccessLevel > AccessLevel.Player && !IsHostile(m) )
                        continue;
