	...
	public class HealerAI : BaseAI
	{
		...
		public override bool Think()
		{
			...
			else
			{
                ...
                if (m_Mobile != null && m_Mobile is Mobile && toHelp != null)
                {
                    int noto = Server.Misc.NotorietyHandlers.MobileNotoriety(((Mobile)m_Mobile), toHelp);
                    isEnemy = (noto == Notoriety.Criminal || noto == Notoriety.Murderer || noto == Notoriety.Enemy);
                }              

				if ( toHelp != null && !isEnemy )
				{
					if ( NeedCure( toHelp ) )
					{
						if ( m_Mobile.Debug )
		...
		{
			return m.Hits < m.HitsMax - 45;
		}

		private static bool NeedLHeal( Mobile m )
		{
			return m.Hits < m.HitsMax - 20;
		}
	}
}