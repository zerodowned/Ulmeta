...
namespace Server.LOS
{
	...
	public class VisibilityMatrix
	{
		...
		public uint[,] Shadows { get { return m_Shadows; } }
		//--------------------------------------------------------------------------
		...
		public void ProcessObstructionAt( Point3D beholder, int x, int y )
		{
			...
			if( maker == null ) return; // no shadows for this obstruction; can
	...
	public partial class LineOfSight
	{
		...
		public bool ItemBlocksVis( Item item, int eyelevel, Point3D beholder )
		{
			...
			//----------------------------------------------------------------------
			if( item.Location.Z > eyelevel ) return false;
			//----------------------------------------------------------------------
			//  multis don't block vis (their parts do, that's different)
			//----------------------------------------------------------------------
			if( item.ItemID >= 0x4000 ) return false;
			//----------------------------------------------------------------------
			//  whitelisted items don't block viz
			//----------------------------------------------------------------------
			if( Config.GetInstance().WhiteListed( item.ItemID | 0x4000 ) ) return false;
			...
			if( Config.GetInstance().Tree( item.ItemID | 0x4000 ) )
			{
				...
				if( treeRange > 0 )
				{
					if( Utility.InRange( item.Location, beholder, treeRange ) ) return false;
					else return true;
				}
				else return false;
			}
		...
		{
			ItemData data = TileData.ItemTable[tile.ID];
			int height = data.CalcHeight;
			int windowRange = LOS.Config.GetInstance().WindowRange;
		...
		public void Warmup()
		{
			...
			Dictionary<int, Point3D> tiles = new Dictionary<int, Point3D>();

			Console.WriteLine( "LOS: WARMING UP" );

			if( !Config.GetInstance().WarmupFacet( m_map.Name ) ) return;
			...
			foreach( Rectangle3D area in areas )
			{
				...
				for( int x = ul.X; x < lr.X; x++ )
				{
					for( int y = ul.Y; y < lr.Y; y++ )
					{
						...
						ItemData landData = TileData.ItemTable[landTile.ID];
						...
						foreach( StaticTile staticTile in staticTiles )
						{
							ItemData staticData = TileData.ItemTable[staticTile.ID];
		...
		public void ReallyWarmup( Dictionary<int, Point3D> tiles )
		{
			...
			foreach( Point3D point in tiles.Values )
			{
				...
				if( count % 2000 == 0 ) Console.WriteLine( " {0}%", count * 100 / tiles.Count );
				else if( count % 25 == 0 ) Console.Write( "." );
			}
